...
# Merge small data frames together
...
# Create a new data frame to calculate the average of variables by day, cricketID, and the stress level for the first 5 minutes
df_new_f <- df_new_f %>%
...
# Create a new dataframe to calculate the average of variables by day, cricketID, and the stress level for the last 5 minutes
df_new_l <- df_new_l %>%
  group_by(Day, CricketID, StressLevel) %>%
...
confint(contrast(model1.emm.s, mtrx1)) # 95% confidence interval
...
LF <- contrast(model1.emm.s, 
               ...
               offset = 0)
LF
confint(LF) # 95% confidence interval


### Second Model
# Dataframe for the second model
